...
FOREACH_CBANG_TYPE(GENERATE_CAST)
...
void ast_print(AST *ast, FILE *fd, unsigned int lvl) {
  ...
  case AST_NUMBER: {
    fprintf(fd, "%*s%s %li\n", lvl * PRINT_INDENT, " ",
            AST_TYPE_STR[AST_NUMBER], ast->data.AST_NUMBER.value);
    return;
  }
  ...
  case AST_FUNC: {
    fprintf(fd, "%*s%s %s -> \n", lvl * PRINT_INDENT, " ",
            AST_TYPE_STR[AST_FUNC], ast->data.AST_FUNC.argName);
    lvl++;
  ...
  case AST_SEXPR: {
    ...
    if (data.right != NULL) {
      ...
      if (data.left != NULL & data.left->tag == AST_BUILTIN) {
        ...
        switch (data.left->data.AST_BUILTIN.ret) {

        case INTEGER: {
          int b =
              CAST_FUNC(data.left->data.AST_BUILTIN.call, int, int, int)(5, 6);
...
enum OperatorType { CBANG_ADD, CBANG_PRINT };
union OperatorFunc {
  int (*add)(int, int);
  void (*print)(char *);
};
struct Operator {
  union OperatorFunc func;
  enum OperatorType type;
};

int add(int a, int b) { return a + b; }

static const struct Operator OPERATORS[] = {{{.add = add}, CBANG_ADD}};
